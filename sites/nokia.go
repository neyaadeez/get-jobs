...
type NokiaMain struct {
	Items []NokiaItem `json:"items"`
...
type NokiaItem struct {
	...
	RequisitionList []NokiaRequisitionList `json:"requisitionList"`
...
type NokiaRequisitionList struct {
	ID         string `json:"Id"`
...
func GetNokiaJobs() ([]common.JobPosting, error) {
	fmt.Println("Processing: ", "Nokia")
	client := common.GetClient()

	url := "https://fa-evmr-saasfaprod1.fa.ocs.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitions?onlyData=true&expand=requisitionList.secondaryLocations,flexFieldsFacet.values,requisitionList.requisitionFlexFields&finder=findReqs;siteNumber=CX_1,facetsList=LOCATIONS%3BWORK_LOCATIONS%3BWORKPLACE_TYPES%3BTITLES%3BCATEGORIES%3BORGANIZATIONS%3BPOSTING_DATES%3BFLEX_FIELDS,limit=1000,lastSelectedFacet=TITLES,selectedLocationsFacet=300000000480126,selectedTitlesFacet=TRA%3BSWA%3BRIN,sortBy=POSTING_DATES_DESC"

	resp, err := client.R().Get(url)
	if err != nil {
		return nil, fmt.Errorf("error creating API request(Nokia jobs): %v", err)
	}

	var nokiaJobs NokiaMain
	err = json.Unmarshal(resp.Body(), &nokiaJobs)
	if err != nil {
		return nil, fmt.Errorf("error parsing json (Nokia jobs): %v", err)
	}

	var jobPostings []common.JobPosting
	for _, job := range nokiaJobs.Items[0].RequisitionList {
		jobPostings = append(jobPostings, common.JobPosting{
			Company:      "Nokia",
			JobId:        common.Nokia + ":" + job.ID,
			JobTitle:     job.Title,
			Location:     job.PrimaryLocation,
			PostedOn:     job.PostedDate,
			ExternalPath: "https://jobs.nokia.com/en/sites/CX_1/job/" + job.ID,
		})
	}
