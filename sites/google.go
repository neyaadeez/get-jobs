...
func GetGoogleJobs() (JobsResponse, error) {
	client := GetClient()
...
func parseJobPage(body []byte) (JobsResponse, error) {
	doc, err := goquery.NewDocumentFromReader(io.NopCloser(bytes.NewReader(body)))
	if err != nil {
		return JobsResponse{}, fmt.Errorf("error parsing the HTML: %v", err)
	}
	...
	return JobsResponse{JobPostings: jobs, Total: count}, nil
}
